...
<!-- Main C    <main class="flex-1 p-gutter md:p-margin-desktop overflow-x-hidden">
        <!-- Header Section -->
        <header class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-xl">
            ...
            <div class="flex items-center gap-3">
                <span class="text-label-md font-label-md text-on-surface-variant">{{ $orders->total() }} total active orders</span>
            </div>
        </header>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-gutter mb-xl">
            @php
                $allOrders = \App\Models\Order::forFarmer(Auth::id())->get();
                $inTransit = $allOrders->whereIn('order_status', ['shipped', 'picked_up'])->count();
                $pendingPrep = $allOrders->whereIn('order_status', ['confirmed', 'paid'])->count();
                $completed = $allOrders->where('order_status', 'delivered')->count();
                $revenue = $allOrders->where('payment_status', 'completed')->sum('total_amount');
            @endphp
            <div class="bg-surface-container-lowest p-lg rounded-[32px] border border-outline-variant/10 flex flex-col gap-2">
                <span class="material-symbols-outlined text-primary text-[32px]">local_shipping</span>
                <div class="mt-4">
                    <p class="text-display-lg font-display-lg text-primary">{{ sprintf('%02d', $inTransit) }}</p>
                    <p class="text-label-md font-label-md text-on-surface-variant">In Transit</p>
                </div>
            </div>
            <div class="bg-surface-container-lowest p-lg rounded-[32px] border border-outline-variant/10 flex flex-col gap-2">
                <span class="material-symbols-outlined text-on-surface-variant text-[32px]">package_2</span>
                <div class="mt-4">
                    <p class="text-display-lg font-display-lg text-on-surface">{{ sprintf('%02d', $pendingPrep) }}</p>
            ...
            <div class="bg-surface-container-lowest p-lg rounded-[32px] border border-outline-variant/10 flex flex-col gap-2">
                ...
                <div class="mt-4">
                    <p class="text-display-lg font-display-lg text-on-surface">{{ sprintf('%02d', $completed) }}</p>
                    <p class="text-label-md font-label-md text-on-surface-variant">Completed</p>
                </div>
            </div>
            <div class="bg-surface-container-lowest p-lg rounded-[32px] border border-outline-variant/10 flex flex-col gap-2 bg-gradient-to-br from-primary/5 to-transparent">
                <span class="material-symbols-outlined text-primary text-[32px]">payments</span>
                <div class="mt-4">
                    <p class="text-display-lg font-display-lg text-primary">₹{{ number_format($revenue / 1000, 1) }}K</p>
                    <p class="text-label-md font-label-md text-on-surface-variant">Revenue (Paid)</p>
                </div>
            </div>
        </div>

        <!-- Orders Table List -->
        <section class="bg-surface-container-lowest rounded-[32px] border border-outline-variant/10 overflow-hidden">
                        ...
                        <tr class="hover:bg-surface-container-low/50 transition-colors cursor-pointer group">
                            ...
                            <td class="px-lg py-5">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-body-md">{{ $order->product->name ?? 'Deleted' }}</span>
                                </div>
                            </td>
                            <td class="px-lg py-5 text-on-surface-variant">{{ $order->buyer->name ?? '—' }}</td>
                            <td class="px-lg py-5">
                                <span class="px-3 py-1 rounded-full text-label-sm font-bold
                                    @if($order->order_status === 'delivered') bg-primary/10 text-primary
                                    @elseif($order->order_status === 'shipped') bg-secondary/10 text-secondary
                        ...
                        <tr>
                            <td colspan="5" class="px-lg py-12 text-center text-on-surface-variant">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="font-body-md">No orders found yet. Keep selling!</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-gutter px-margin-desktop max-w-[1440px] mx-auto">
        ...
        <div class="flex flex-col gap-4">
            ...
            <div class="flex gap-4 mt-2">
                <span class="material-symbols-outlined text-primary cursor-pointer hover:opacity-70 transition-opacity">share</span>
                <span class="material-symbols-outlined text-primary cursor-pointer hover:opacity-70 transition-opacity">language</span>
            </div>
        </div>
    </div>
